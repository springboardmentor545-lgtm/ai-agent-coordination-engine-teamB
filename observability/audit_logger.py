...
def log_event(thread_id, agent_name, action, status="success",
              employee_id=None, tool_name=None, duration_ms=None, detail=None, http_status=None):
    """
    ...
    code = f" [{http_status}]" if http_status is not None else ""
    tag = " [FAILED]" if status == "failure" else ""
    note = f" - {detail}" if detail else ""

    message = f"[{short_id}] {agent_name} -> {label}{timing}{code}{tag}{note}"
    if status == "failure":
